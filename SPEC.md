...
Git remote (SSH): `https://github.com/AZagatti/pokedex-s46hi-opusadv.git` (already created, empty). `git init`, set remote `origin`, commit in logical chunks, push to `main`. `paths.base` must be `/pokedex-s46hi-opusadv`. The live URL will be `https://azagatti.github.io/pokedex-s46hi-opusadv/`.
